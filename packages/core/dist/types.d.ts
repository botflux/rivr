...
export type StepResult<State> = Success<State> | Failure | Skipped | Stopped;
 ...
 */
export type MergeUnionTypes<T> = (T extends any ? (x: T) => any : never) extends (x: infer R) => any ? R : never;
...
export declare function rivrPlugin<Out, Opts = undefined, State = any, Deps extends RivrPlugin<any, any, State>[] = []>(plugin: (w: ReadyWorkflow<State, MergeUnionTypes<GetDecorator<UnwrapItem<Deps>, State>>>, opts: Opts) => Workflow<State, Out>, opts: RivrPluginOpts<State, Deps>): RivrPlugin<Out, Opts, State>;
